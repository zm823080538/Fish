...
{
    if (IOS11) {
        // 修复iOS11下，按钮不能改变状态的问题
        if ([self isMinimumSelectionLimitFulfilled]) {
            UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithTitle:@"完成" style:UIBarButtonItemStylePlain target:self action:@selector(done:)];
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL reached = [self isMaximumSelectionLimitReached];
    if (reached) {
        [UIView showErrorToast:[NSString stringWithFormat:@"您最多只能选择%lu张照片", self.imagePickerController.maximumNumberOfSelection]];
    }
    return !reached;
}
